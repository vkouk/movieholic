                                ...
                                this.props.order.movies && this.props.order.movies.length >= 1 ? this.props.order.movies.map(movie => {
                                    return (
                                        <li key={movie._id} className="flex items-center lh-copy pa3 ph0-l bb b--black-10">
                                            <img className="w3 h3 w3-ns h3-ns br-100" src={movie.poster} alt={movie.title} />
                                            <div className="pl3 flex-auto">
                                                <span className="f6 db black-70 title">{movie.title}</span>
                                                <span className="f6 db black-70 genre">{movie.genre}</span>
                                            </div>
                                            <div>
                                                <span className="f6 link blue price">Price: {movie.rating ? returnRentProductCost(movie.rating, this.props.order.dateOrdered) : 0}</span>
                                            </div>
                                        </li>
                                    )
                                ...
                                this.props.order.series && this.props.order.series.length >= 1 ? this.props.order.series.map(serie => {
                                    return (
                                        <li key={serie._id} className="flex items-center lh-copy pa3 ph0-l bb b--black-10">
                                            <img className="w3 h3 w3-ns h3-ns br-100" src={serie.poster} alt={serie.title} />
                                            <div className="pl3 flex-auto">
                                                <span className="f6 db black-70 title">{serie.title}</span>
                                                <span className="f6 db black-70 genre">{serie.genre}</span>
                                            </div>
                                            <div>
                                                <span className="f6 link blue price">Price: {serie.rating ? returnRentProductCost(serie.rating, this.props.order.dateOrdered) : 0}</span>
                                            </div>
                                        </li>
                                    )
