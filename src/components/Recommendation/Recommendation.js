...
class Recommendation extends Component {
  ...
  renderContent = () => {
    ...
    if (
      ...
      (recommendations.suggestedMovies.length === 0 &&
        recommendations.suggestedSeries.length === 0)
    ) {
        ...
        <div className="container">
          <div className="row pt-4 pb-4">
            <div className="col-sm-6 text-center">
              Please{" "}
              <span className="btn btn--home" onClick={() => onSelectChange(0)}>
                Login
              </span>{" "}
              /{" "}
              <span className="btn btn--home" onClick={() => onSelectChange(1)}>
                Register
              </span>{" "}
              to browse movieholic.
            </div>
            <div className="col-sm-6">
              {selectedIndex === null ? null : selectedIndex === 0 ? (
                <Login />
              ) : (
                <Register />
              )}
        ...
        <div className="container-fluid">
          <div className="row pt-2">
            <div className="col-sm-12 text-center">
              <p>
                No recommendations found for you. Visit{" "}
                <Link to={"/movies"} className="btn btn--home">
                  Movies
                </Link>{" "}
                or{" "}
                <Link to={"/series"} className="btn btn--home">
                  Series
                </Link>{" "}
                and rent some.
              </p>
...
export default connect(
  mapStateToProps,
  { getUserRecommendations, clearRecommendations }
