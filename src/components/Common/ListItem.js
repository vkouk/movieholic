...
class ListItem extends Component {
  ...
  onSortBy = label => {
    ...
    if (label === "Title") {
      sortedData = this.state.currentData.sort((a, b) =>
  ...
  render() {
    const { currentPage, totalPages, currentData, title } = this.state;
    ...
    return (
      <div className="list">
        <div
          className={`${
            this.props.isHeaderVisible
              ? "list__header  list__header--small"
              : "list__header"
          }`}
        >
          <div className="list__header__text">
            {this.props.location === "/"
              ...
              : _.capitalize(this.props.location.slice(1))}
          </div>
        ...
        <div className="list__row">
          {currentData
            .filter(
              rec => rec.title.toLowerCase().search(title.toLowerCase()) !== -1
            )
            .map(record => {
              return (
                <Item
