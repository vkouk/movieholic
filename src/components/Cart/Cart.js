...
class Cart extends Component {
  componentDidMount() {
    ...
    if (
      ...
      this.props.cart.movie.length <= 0 &&
  ...
  onButtonClick = e => {
    ...
    const cartMovies = this.props.cart.movie
      ? this.props.cart.movie.map(data => data._id)
      : [];
    const cartSeries = this.props.cart.serie
      ? this.props.cart.serie.map(data => data._id)
      : [];

    this.props.storeRent(
      {
...
export default PrivateRoute(
  connect(
    mapStateToProps,
    { getCart, storeRent, removeMovieFromCart, removeSerieFromCart }
  )(Cart)
);
