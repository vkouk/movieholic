...
export default ({
  ...
  const cartTotalAmount = cart.movie
    ? cart.movie.length
    : 0 + cart.serie
    ? cart.serie.length
    : 0;
    ...
    <div className={`cart ${isCartVisible ? "is-visible" : ""}`}>
      <div className="cart__header">
        ...
        <div onClick={onCartToggle}>
          <FontAwesomeIcon icon={faTimes} className="fa-lg cart__icon" />
        </div>
      </div>
      ...
      <div className="cart__footer">
        <div className="cart__footer__item">
          <button
            className={`cart__btn ${cartTotalAmount >= 1 ? "" : "is-disabled"}`}
            onClick={e => {
              if (error) {
                return error;
              } else {
        ...
        <div className="cart__footer__item">
          <h2 className="f6 fw4 mt2 mb0 black-60">
            ...
            {returnRentTotal(cart.movie, cart.serie, new Date())}€
          </h2>
        </div>
        <div className="cart__footer__item">
